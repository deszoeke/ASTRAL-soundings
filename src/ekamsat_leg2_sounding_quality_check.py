...
for f_name in file_list:
    ...
    if f_name.endswith('.nc'): # read only nc files
        ...
        start_h = 3
        data = np.array([height[start_h:], P[start_h:], ws[start_h:], wd[start_h:], T[start_h:], Td[start_h:], rh[start_h:]])
        ...
        [num_error, lim_error, ord_error,minH, maxH,
         cape,cin,pw,el,lcl,lfc, MLD] =  sqf.qc_check(df_data, f_name, path_in)
