% Simon's code for calculating quantities from the sounding array during
% MISOBOB 2019

% cd('/Users/sdeszoek/Data/cruises/MISOBOB_2019/SR1911/scientific_analysis/programs/radiosonde')
% cd('/Volumes/cruise/SR1911/scientific_analysis/programs/radiosonde')
...
% addpath(genpath('/Users/sdeszoek/Documents/MATLAB/user_tools/graphics/m_map/'));
%thermo_constants;

paren = @(X, varargin) X(varargin{:});

update = false;
if update     % make sure internet is on first
    ...
    % rsync_ride = 'rsync -parv /Volumes/cruise/SR1911/share/data/radiosonde/ ~/Data/cruises/MISOBOB_2019/SR1911/radiosonde/';
    
    system(fetch_uwyo)
    % system(rsync_ride) % versnon of record is now on laptop
...
datadir = '~/Documents/WHOI/Projects/ASTraL/2024_IOP1/ASTRAL-Soundings/data/';
...
% 6 h time grid;
year = 2019;
mday6h = datenum(year,7,10):0.25:min(now,datenum(year,8,1));
...
clear u v temp hght shum
trunc = @(x) x; % identity function; was x(1:end-1); 
for istn = 1:length(Stn)
    for it = 1:nt
        u(:,it,istn)    = trunc( binavg( plev, A(1).p, B(istn).uwnd(:,it) ) ); % time, height
        v(:,it,istn)    = trunc( binavg( plev, A(1).p, B(istn).vwnd(:,it) ) );
        temp(:,it,istn) = trunc( binavg( plev, A(1).p, B(istn).temp(:,it) ) ); % K
        hght(:,it,istn) = trunc( binavg( plev, A(1).p, B(istn).hght(:,it) ) ); % geopotential height
        shum(:,it,istn) = trunc( binavg( plev, A(1).p, B(istn).shum(:,it)*1e-3 ) ); % kg/kg
    end
end
u = u(1:end-1,:,:);
v = v(1:end-1,:,:);
