...
# cd /Users/sdeszoek/Data/cruises/PISTON_MISOBOB_2018/Soundings/Palau/91408_KororPalau
stn_names = ['Bhubaneswar', 'Chennai', 'Kolkata', 'PortBlair', 'Thiruvanathapuram', 'Vishakhapatnam']
data_path_stem = '/Volumes/cruise/SR1911/share/data/radiosonde/'

#fileyears=range(2018,2018+1)
#filemonths=range(5,8+1)
fileyears = range(2019, 2020) # just 2019, excludes stop year
filemonths = range(6,9)
...
# composite soundings by a phase index
def compsond(ph,x):
    nh=np.shape(x)[1]
...
# loop over all station, year, month files
for istn in range(1,7):
    for fy in fileyears:
        for fm in filemonths:
            filename = data_path_stem + stn_names[istn] + "{:0>4d}{:0>2d}.txt".format(fy,fm)
            print(filename)
    ...
    # create a dictionary and save as a mat file
    Soundings = {'station_num':station_num_id, 'station_wban':station_wban_id, \
                 'station_name':station_name, 'datenumber':datn, 'pres':pres, \
                 'hght':hght, 'temp':temp, 'dwpt':dwpt, \
                 'relh':relh, 'mixr':mixr, 'drct':drct, \
                 'sknt':sknt, 'thta':thta, \
                 'thte':thte, 'thtv':thtv, 'shum':shum, \
                 'uwnd':uwnd, 'vwnd':vwnd }
    io.savemat(data_path_stem + stn_names[istn] + stn_names[istn] + '_soundings', Soundings)
    ...
    ax[0].plot(np.nanmean(u,0), ht); ax[1].set_xlabel('zonal velocity (m/s)')
    ax[1].plot(np.nanmean(v,0), ht,  title='meridional velocity (m/s)')
    ax[2].plot(np.nanmean(th,0),ht,  title='potential temperature (m/s)')
    ax[3].plot(np.nanmean(qv,0)*1e3,ht,  title='specific humidity (g/kg)')

    ax[0].plot(u.transpose(), ht); ax[1].set_xlabel('zonal velocity (m/s)')
    ax[1].plot(v.transpose(), ht,  title='meridional velocity (m/s)')
    ax[2].plot(th.transpose(),ht,  title='potential temperature (m/s)')
    ax[3].plot(qv.transpose()*1e3,ht,  title='specific humidity (g/kg)')

    plt.show()

# Plot anomaly time heights

## Read in RMM index, for example
#
...
#rmmpath='//indian/cruiseshare/indices/RMM/'
