...
clear
cd('/Users/sdeszoek/Data/cruises/MISOBOB_2019/SR1911/scientific_analysis/programs/radiosonde/thorpe_andrea')
...
datdir = '/Users/sdeszoek/Data/cruises/MISOBOB_2019/SR1911/radiosonde/Ride/';
...
% (/100) as input. Now passed as input into the eddy dissipation function
% instead of calling within the function to increase speed (readtable function takes ~0.05s)
tnrmin = wilson2010_lookup(plev);
...
for ifile = 1:nfiles
    ...
    [theta_v(ifile,:), epsi(ifile,:), K(ifile,:)] = ...
        eddyDiffusivityThorpe(theta, q, z, z2, tnrmin, mixing_eff);
...
if savefig
    figname = fullfile(figdir, 'thorpe_epsilon.jpg');
    export_fig(figname, '-r400')
end
...
if savefig
    figname = fullfile(figdir, 'thorpe_epsilon_wclouds.jpg');
    export_fig(figname, '-r200')
    figname = fullfile(figdir, 'thorpe_epsilon_wclouds.png');
    export_fig(figname, '-r200')
...
if savefig
    figname = fullfile(figdir, 'thorpe_epsilon_stats.jpg');
    export_fig(figname, '-r200')
end
...
function [theta_v, epsi, K] = eddyDiffusivityThorpe(T, q, z, z2, tnrmin, mixing_eff)
    % Input variables
...
%     dtheta_dz = forwDiff_v(theta_v_sort, z2(2) - z2(1));
    dtheta_dz = [diff(theta_v_sort) NaN] / (z2(2) - z2(1)); % forwDiff_v ??
